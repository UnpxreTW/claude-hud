...
test('render does not treat a real 40-column terminal as unknown maxWidth fallback', () => {
  ...
  ctx.config.display.showUsage = false;
  ctx.stdin.context_window.current_usage.input_tokens = 0;
  ctx.extraLabel = '1234567890123456789012345678901';
  ...
  assert.ok(lines[0].includes('1234567890123456789012345678901'), 'full extra label should remain visible at the real terminal width');
...
test('render does not strand a bare 5h continuation line in compact mode', () => {
  ...
  assert.ok(lines.some(line => line.includes('Usage 5h') && line.includes('30 %')), `expected usage window to keep its label: ${lines.join(' | ')}`);
