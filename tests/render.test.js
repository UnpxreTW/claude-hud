...
test('renderSessionLine shows bar at 100% usage instead of warning', () => {
  const ctx = baseContext();
...
test('renderUsageLine shows bar at 100% with reset countdown', () => {
  const ctx = baseContext();
  const resetTime = new Date(Date.now() + (151 * 3600000) + (59 * 60000)); // 6d 7h 59m from now
  ctx.usageData = {
    planName: 'Pro',
...
test('renderSessionLine shows 100% usage with custom colors', () => {
  const ctx = baseContext();
  ctx.config.colors = {
    context: 'green',
    usage: 'brightBlue',
  ...
  const line = stripAnsi(renderSessionLine(ctx));
  assert.ok(line.includes('100 %'), 'should show 100% usage normally');
});
...
test('renderUsageLine uses custom usage palette overrides', () => {
  ...
  assert.ok(weeklyLine.includes('\x1b[35m 80 %\x1b[0m'), `expected custom weekly usage warning percentage color, got: ${JSON.stringify(weeklyLine)}`);
});
...
test('render expanded layout combines default merge-group elements when adjacent in elementOrder', () => {
  ...
  assert.ok(!stripped.includes('Usage  '), `combined line should not pad the usage label: ${stripped}`);
});
