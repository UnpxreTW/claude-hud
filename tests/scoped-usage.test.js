...
test('renderUsageLine renders scoped-only usage without a ghost generic window', () => {
  ...
  // Fork override: percentages are padded to 3 chars followed by " %".
  assert.match(line, /Usage\s+Fable\s+38 %/);
...
test('renderSessionLine includes scoped usage and preserves a custom usage color', () => {
  ...
  assert.match(stripAnsi(line), /Usage\s+Fable\s+38 %/);
  assert.match(line, /\x1b\[36m 38 %\x1b\[0m/);
});

test('a maxed-out window renders a full bar (no limit warning) and retains bounded scoped usage in both layouts', () => {
  const usage = scopedUsage({ fiveHour: 100 });
  const ctx = renderContext(usage);

  // Fork override: no limit-reached branch — 100% renders normally, and the
  // model-scoped window is still appended.
  const expanded = stripAnsi(renderUsageLine(ctx) ?? '');
  const compactLayout = stripAnsi(renderSessionLine(ctx));
  assert.match(expanded, /100 %.*Fable\s+38 %/);
